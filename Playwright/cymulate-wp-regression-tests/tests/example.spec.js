...
function normalizeName(url) {
  return url
    .replace('https://cymulate.com/', '')
    .replace(/\//g, '-')
    .replace(/^-|-$/g, '') || 'home';
}

async function preparePageForScreenshot(page) {
  // Disable animations and hide known dynamic elements
...
test.describe('cymulate.com — visual regression (stable pages)', () => {
  for (const url of pages) {
    ...
    test(name, async ({ page }) => {
      ...
      await expect(page).toHaveScreenshot(`cymulate-${name}.png`, {
        fullPage: true,
